...
class DietFiltersView extends View {
  ...
  render(data = {}, render = true) {
    ...
    try {
      ...
      const dropdown = this._parentElement.querySelector(
        '.diet-filters__dropdown'
      );
      if (dropdown) {
        dropdown.addEventListener('click', function (e) {
          e.stopPropagation();
          document
            .querySelector('.diet-filters__menu')
  ...
  toggleActiveClass(filterType, isActive) {
    ...
    const checkbox = this._parentElement.querySelector(
      `.diet-filters__checkbox[data-filter="${filterType}"]`
    );
    if (!checkbox) {
    ...
    console.log(
      `Toggling ${filterType} to ${isActive ? 'active' : 'inactive'}`
    );
